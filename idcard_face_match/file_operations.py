...
from idcard_face_match.asn1 import len2int

class EFReadError(Exception):
    """Raised if an error occurs during data read from a file."""

def get_size_from_ef(q_main: Queue, sm_object: SMObject, fid: bytes, fname: str) -> bytes:
    """
    Read the first 4 bytes from the file id fid and return together with file length

    sm_object -- Necessary secure messaging object (Encryption session key etc.)
    fid -- File id
    fname -- Printed file name
    :returns: first 4 data bytes from the file; file size
    """
    # Select File
...
def read_data_from_ef(q_main: Queue, sm_object: SMObject, fid: bytes, fname: str) -> bytes:
    """
    ...
    data, data_len = get_size_from_ef(q_main, sm_object, fid, fname)
    offset = 4
    ...
    blocksize = 0xe7 # 231
    # old comment - 250 because all the cards I have access to only send 250 bytes in BAC SM 242 in AES SM

    blocksize = 164 # use a small block size in order to obtain more granual progress bar update

    # while we have not downloaded the entire file
    while offset < data_len:

        le = bytes([ min(blocksize, data_len - offset) ])

        # exception caught in main program loop
        decrypted_data = send(sm_object, APDU(b"\x00", b"\xB0", bytes([offset >> 8]), bytes([offset & 0xFF]), Le=le))
