...
from idcard_face_match.asn1 import dump_asn1, encode_oid_string, get_digestalg_name
...
def passive_auth(
    ...
    if certificates is not None:
        ...
        print("[+] Document Signer Certificate: %s (notBefore: %s, notAfter:%s)" % (CDS.get_subject(), CDS.get_notBefore(), CDS.get_notAfter()))
        print("[+] CSCA Certificate (to be found): %s)" % (CDS.get_issuer()))
        ...
        try:
            ...
            # OpenSSL v3.x path validation rejects certificates with explicit ECC parameters
            # in case of explicit ECC parameters, we try to verify signature manually
            if str(ex) == 'Certificate public key has explicit ECC parameters':
                print("[+] Document Signer Certificate has explicit ECC parameters. Trying to verify signature manually...")
                # looking for CSCA cert that issued DS cert
                CSCA_found = False
                for cert in build_store.certs:
                    if cert.get_subject()==CDS.get_issuer():

                        CSCA_found = True
                        print("[+] CSCA cert found:", cert.get_subject())

                        # get root node
                        i = asn1_node_root(doc_sig_cert)
                        # get 1st item inside Certificate (tbsCertificate sequence)
                        i = asn1_node_first_child(doc_sig_cert, i)
                        tbsCertificate = asn1_get_all(doc_sig_cert, i)
                        # get 2nd item inside Certificate (signatureAlgorithm)
                        i = asn1_node_next(doc_sig_cert, i)
                        # get 3rd item inside Certificate (SignatureValue)
                        i = asn1_node_next(doc_sig_cert, i)
                        signature = asn1_get_value_of_type(doc_sig_cert, i, "BIT STRING")[1:]
                        hash_alg = CDS.get_signature_algorithm().replace(b'ecdsa-with-', b'').decode()
                        try:
                            if verify(cert, signature, tbsCertificate, hash_alg) is None:
                                print("[+] The signature on DS is valid.")
                        except:
                            exceptions.append(PassiveAuthenticationError("[-] The signature on DS is not valid or failed!"))

                if not CSCA_found:
                            exceptions.append(PassiveAuthenticationError("[-] CSCA cert for DS cert not found!"))
            else:
                exceptions.append(PassiveAuthenticationError("[-] Document Signer Certificate is not signed by a CSCA certificate or is invalid!\n" + str(ex)))
    else:
        raise PassiveAuthenticationCriticalError("[-] This application doesn't support this kind of document yet!")
